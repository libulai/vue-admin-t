  ...
  export default {
    ...
    methods: {
      async querySearch(queryString, cb) {
        let rs = await this.initPlots(queryString)
        cb(rs);
      },
      handleSelect(item) {
      ...
      async sphone(val) {
        ...
        if (rs.data.length && rs.data.length > 0) {
          ...
          this.form.customername = this.phone.contacter
          this.form.customertype = this.phone.customertype
        } else {
          this.phone = {}
        }
      },
      ...
      async initPlots(name) {
        let rs = await this.$http({
          url: `/admin/communitylist?commnuityname=${name}`,
          method: "get"
        });
        let arr = []
        rs.data.forEach(i => {
          arr.push({
            value: i.communityname,
            // address
          })
        })
        return arr
      ...
      reset() {
        ...
        this.form.Receipt20 = 0
        this.form.Receipt21 = 0
        this.form.Receipt22 = 0
      ...
      async save() {
        this.$refs.form.validate(async (valid) => {
          if (valid) {
            let form = this.form
            if (form.Receipt20 == 0 && form.Receipt21 == 0 && form.Receipt22 == 0 && form.Receipt23 == 0) {
              this.$message({
                message: '户型必须至少选择1个房间',
                type: 'error'
              })
              return
            }
